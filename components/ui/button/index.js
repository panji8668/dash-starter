...
const XButton = React.forwardRef((props, ref) => {
  const {
    children,
    size,
    color,
      ...
      case SIZES.SM:
        sizeClass = classNames(
          ...
          icon && !children
            ...
            : `${sizeIconClass}  px-3 py-2 text-sm`,
        )
        break
      case SIZES.XS:
        sizeClass = classNames(
  ...
  const solidColor = () => {
    const btn = {
      ...
      hoverColor: active ? 'shadow-sm' : `hover:bg-${buttonColor}-${decreaseLevel}`,
  ...
  const plainColor = () => {
    const btn = {
      ...
      textColor: `text-gray-600 dark:text-gray-100`,
      hoverColor: active ? 'shadow-sm' : `hover:bg-gray-50 dark:hover:bg-gray-600`,
      activeColor: `active:bg-gray-100 dark:active:bg-gray-500 dark:active:border-gray-500`,
    }
    return getBtnColor(btn)
  }
